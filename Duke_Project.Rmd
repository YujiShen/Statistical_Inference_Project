...
author: "By YUJI SHEN | APRIL 15, 2015"
output:
  html_document:
...
* The most dramatic change is on Homosexual Sex. People change their attitude almost by 1 degree, which means the liberal level upgraded!
* Another ascender is Premarital Sex. Its change is not as much as the former, but rising steadily. And its liberal level keeps highest among all these issues.
...
Besides the trends over time, here are also trends on education degree. The people received higher education will have a lower proportion of thinking these sexual ethics as "Always Wrong". This plot seems to convince us that the education will make people become liberal. Is this correct? Next, I will focus on extramarital sex to prove that *__1. there is a relationship between opinion on  extramarital sex and education degree, and 2. this lower proportion are significant on statistics__*.
...
This is a test for independence between two categorical variables (each has more than 2 levels), so **_Chi-squared Test for Independence_** will be applied.
...
* ___Independence___: This is a random sampling survey with 33397 records after omitting missing values which is less than 10% of population of U.S. Also, each respondents can only choose one answer. The conditions for independence are satisfied.
* ___Sample Size___: The least number in summary table is 52 (see output below), so each particular scenario have more than 5 cases. The condition for sample size is satisfied.
...
* ___Independence___
    * ___Within Groups___: This is a random sampling survey with 6947 records after omitting missing values which is less than 10% of population of U.S. So the sampled observations are independent within people with different education degree.
...
A 0 p-value implies that the proportion of "Always Wrong" in Bachelors is significantly greater than the  proportion of "Always Wrong" in Graduates. Thus we can infer graduates give more accepting for extramarital sex.

But this is only a test for two groups in education, then I applied the same test on the rest groups following their orders. The result shows that except "Junior College" and "High School" have a 0.2 p-value, all relative lower education degree groups have significant higher proportions of "Always Wrong". Therefore, to some degree, we can say *__better educated americans are more liberal for extramartial sex__*.

## 5. Liberal ≠ Unruly

However, being liberal about this issue does not equal to the actual performance of people. So the reasonable one would be using the data about the behavior of people's out of marriage affairs. And there really is one - [Fair's Extramarital Affairs Data (Table F17.2) ^2^](http://pages.stern.nyu.edu/~wgreene/Text/Edition7/tablelist7.htm). 

I will use the second survey in Fair's Affairs Data, which is conducted by Redbook magazine in 1974 and female  respondents only. A questionnaire on sex was published in the October issue of Redbook, and readers were asked to mail in their answers. Although it was not random sampled, what I concern is the distribution of extramarital sex within each group. So this bias seems not a big issue here.
